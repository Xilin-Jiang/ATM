...
![My Image](man/figures/ATM_schematic.png)
...
## Update log
Feb 23rd, 2026 (in development version): using data.table to perform the initial filtering of duplicated diagnosis which is much faster; also makes the inference slightly faster by simplify the row id data structure (row id list was designed to optimise the speed; now it is slightly faster as the software only save a vector instead of a dataframe).  

Nov 5th, 2025: If there are records in the age information that is ouside the range (0,90], we will throw an informative error and give suggestions to use `age_imputation`.
Please note, this also applies the `loading2weights`; please note `loading2weights` is a one-step function for getting weights for GWAS or "comorbidity weights".

September 15th, 2025: ATM now can be run when age information is not available. In that case ATM is reduced to the LDA model. 
As the LDA model are implemented using collapsed variational inference, it theoretically are more accurate than most implementation that uses mean field variational inference. 
...
ATM is available as "AgeTopicModels" on CRAN. You can install and apply the model.
```r
install.packages("AgeTopicModels")
library(AgeTopicModels)
```

(*preferred) You can also install the development version of ATM from [GitHub](https://github.com/ATM). This version contains the newest updates to the software. 
The advantage of development version is that (1) it contains a larger example data and (2) it allows ATM to run with automatic data save; these properties are helpful when you need to run ATM on large data sets. Both of these properties are not permitted by CRAN. 
...
ATM_results <- wrapper_ATM(rec_data_imputed, 10, CVB_num = 1)
```



## Internal simulated data example
...
If there are **no age information**, ATM will degrade to LDA, please use this visualisation instead.
```r
...
![My Image](man/figures/ATM_generative_process.png)
...
![My Image](man/figures/ATM_LFA_comparison.png)
