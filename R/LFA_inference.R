...
topic_init_lfa_cvb <- function(rec_data, ds_list, topic_num){
  ...
  message("LFA RAM occupation: ")
  message(object.size(para), unit = "GB" , standard = "SI")
...
wrapper_LFA <- function(rec_data, topic_num, CVB_num = 5, save_data = FALSE, beta_prior_flag = FALSE, topic_weight_prior=NULL){
  ds_list <- rec_data %>%
    group_by(diag_icd10) %>%
    summarise(occ = n())
  topics <- list()
  ...
  for(cvb_rep in 1:CVB_num){
    ...
    if(is.null(topic_weight_prior)){
      ...
      if(length(topic_weight_prior) != para$K){
        message("pre-specified prior of topic weight must be a postive vector of length K (number of topics)")
        para$alpha <- rep(1, para$K)
      }else{
...
plot_lfa_topics <- function(disease_names, beta,  plot_title = ""){
  ...
  if(length(disease_names) <= 50){
    ...
    message("Too many diseases (>50), hide the disease name.")
    plt <- ggplot() +
      ggplot2::geom_tile(data = longData, aes(x = Var2, y = Var1, fill=red, alpha = value,width = 0.9)) +
      ggplot2::scale_alpha_continuous(range = c(0,1)) +
      ggplot2::labs(x="", y="", title="") +
