...
for(rep_id in 1:rep_number){
  ...
  ###########################################
  maf_ukb_GWAS <- read.csv("maf_ukb_GWAS.csv")
  allele_frequency <- sample(size = 100, x = maf_ukb_GWAS$MAF, replace = T)
...
save(genetic_simulation, file =  paste0("../Results_NG_revision/", "realmaf_gen_simulation_v",v_id,"s",s_id,".RData"))
