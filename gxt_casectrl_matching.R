...
# filter disease
#################################
topic_specific_set <- para$unlist_Ds_id[para$ds_list[[j]]$id,]
CasePCA <- data_pca %>%
...
for(itr in 1:(num_itr+1)){
  ...
  dist_mat <- ControlPCA %>%
    select(eid) %>%
    cbind(dist_mat)
  if(as.numeric(ds_id) == 401.1){ # for hypertension only do 1:1 matching
...
write.table(control_for_gwas, paste0("/users/mcvean/xilin/xilin/Multimorbidity-biobank/GxTopic/", ds_id,"/", ds_id, "_control.txt",sep=""), sep="\t", col.names = FALSE, row.names = FALSE)
write.table(case, paste0("/users/mcvean/xilin/xilin/Multimorbidity-biobank/GxTopic/", ds_id, "/", ds_id, "_case.txt",sep=""), sep="\t", col.names = FALSE, row.names = FALSE)
write.table(DataGWAS, paste0("/users/mcvean/xilin/xilin/Multimorbidity-biobank/GxTopic/", ds_id, "/", ds_id, "_pheno_over_age.txt",sep=""), sep="\t", col.names = FALSE, row.names = FALSE)
