...
para$rep_ID <- args[3]
set.seed(19940110 + as.numeric(para$rep_ID))
############################
# start optimization
############################
...
for(itr in 1:para$max_itr){
  ...
  if(itr %% para$itr_save ==0){
    save(para, file = paste0("Results/","Run_2rec_PheCode_age_dependent_K",para$K,"_P",para$P,"_rep",para$rep_ID,"_intermediate",itr, ".RData"))
  }
}

# save the parameter
save(para, file = paste0("Results/","Run_strongRegularization_PheCode_age_dependent_K",para$K,"_P",para$P,"_rep",para$rep_ID, ".RData"))
...
print(object.size(model_output), unit = "MB" , standard = "SI")
  save(model_output, file = paste0("Results/","strongRegularization_model_output_PheCode_age_dependent_K",para$K,"_P",para$P,"_rep",para$rep_ID, ".RData"))
